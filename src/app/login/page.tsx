...
export default function CustomerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      if (!result.success) { setError("البريد الإلكتروني أو كلمة المرور غير صحيحة"); return; }
      router.push("/");
    }, 700);
  };
    ...
    <div dir="rtl" style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <div className="auth-brand" style={{
        ...
        <div style={{ position: "relative", zIndex: 2, textAlign: "center", maxWidth: "400px" }}>
          ...
          <h1 style={{ fontSize: "2.2rem", fontWeight: 900, color: "#fff", marginBottom: "0.75rem", lineHeight: 1.3, animation: "fadeInUp 0.8s ease-out 0.1s both" }}>رحلتك تبدأ من هنا</h1>
          <p style={{ fontSize: "1.05rem", color: "rgba(255,255,255,0.8)", marginBottom: "2.5rem", lineHeight: 1.8, animation: "fadeInUp 0.8s ease-out 0.2s both" }}>اكتشف جداول قطارات الرياض واحجز تذكرتك بسهولة وأمان</p>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", animation: "fadeInUp 0.8s ease-out 0.3s both" }}>
            {[{ i: "🔒", t: "حجز آمن ومضمون" }, { i: "⚡", t: "تأكيد فوري للتذاكر" }, { i: "🗺️", t: "تغطية جميع محطات الرياض" }].map((f, idx) => (
      ...
      <div className="auth-form" style={{ flex: "1 1 50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: "3rem 2rem", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: "420px", animation: "fadeInUp 0.6s ease-out" }}>
          <h2 style={{ fontSize: "1.85rem", fontWeight: 800, color: "#0f172a", marginBottom: "0.4rem" }}>تسجيل الدخول</h2>
          <p style={{ color: "#64748b", fontSize: "0.95rem", marginBottom: "1.75rem" }}>أهلاً بعودتك! سجّل دخولك للمتابعة</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
            ...
            <div>
              <label htmlFor="customer-email" style={{ display: "block", fontSize: "0.85rem", fontWeight: 700, color: "#374151", marginBottom: "0.5rem" }}>البريد الإلكتروني</label>
              <div style={{ position: "relative" }}>
                ...
                <input id="customer-email" type="email" dir="ltr" placeholder="example@email.com" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} onFocus={focusIn} onBlur={focusOut} required />
              </div>
            </div>

            <div>
              <label htmlFor="customer-password" style={{ display: "block", fontSize: "0.85rem", fontWeight: 700, color: "#374151", marginBottom: "0.5rem" }}>كلمة المرور</label>
              <div style={{ position: "relative" }}>
                <span style={{ position: "absolute", right: "14px", top: "50%", transform: "translateY(-50%)", fontSize: "1.05rem", opacity: 0.4, pointerEvents: "none" }}>🔑</span>
                <input id="customer-password" type={showPw ? "text" : "password"} dir="ltr" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} style={{ ...inputStyle, paddingLeft: "2.75rem" }} onFocus={focusIn} onBlur={focusOut} required />
                <button type="button" onClick={() => setShowPw(!showPw)} style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: "0.95rem", opacity: 0.5, padding: "4px" }} aria-label="toggle password">{showPw ? "🙈" : "👁️"}</button>
              </div>
            </div>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.85rem" }}>
              <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", color: "#64748b" }}>
                <input type="checkbox" style={{ accentColor: "#14b8a6", width: "16px", height: "16px" }} /> تذكرني
              </label>
              <Link href="/forgot-password" style={{ color: "#0f766e", fontWeight: 600, textDecoration: "none" }}>نسيت كلمة المرور؟</Link>
            </div>

            <button id="customer-login-btn" type="submit" disabled={loading} className="auth-btn" style={{
            ...
            >
              {loading ? <span style={{ display: "flex", alignItems: "center", gap: "0.5rem", justifyContent: "center" }}><span className="auth-spinner" />جاري التحقق...</span> : "تسجيل الدخول"}
            </button>
          </form>
          ...
          <p style={{ textAlign: "center", fontSize: "0.9rem", color: "#64748b", margin: 0 }}>
            ليس لديك حساب؟{" "}
            <Link href="/register" style={{ color: "#0f766e", fontWeight: 700, textDecoration: "none" }}>إنشاء حساب جديد</Link>
          </p>
          <p style={{ textAlign: "center", marginTop: "1rem", fontSize: "0.8rem" }}>
            <Link href="/admin/login" style={{ color: "#94a3b8", textDecoration: "none", fontWeight: 500 }}>🔐 دخول لوحة الإدارة</Link>
          </p>
        </div>
      </div>
